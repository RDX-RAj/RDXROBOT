...
from RDXROBOT import LOGGER
from RDXROBOT import dispatcher as n
from RDXROBOT.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
class MukeshTelegramHandler:
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[RDXROBOT-Msg] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[RDXROBOT-Callback] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
