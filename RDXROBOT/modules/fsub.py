...
from RDXROBOT import BOT_ID
from RDXROBOT import DRAGONS as DEVS
from RDXROBOT import OWNER_ID
from RDXROBOT import telethn as rdx
from RDXROBOT.events import Mukeshinline
from RDXROBOT.events import register as Mukeshbot
from RDXROBOT.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await rdx(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await rdx(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
