...
# from RDXROBOT.modules.sql import warns_sql as warnssql
import RDXROBOT.modules.sql.blacklist_sql as blacklistsql

# from RDXROBOT.modules.sql import cust_filters_sql as filtersql
# import RDXROBOT.modules.sql.welcome_sql as welcsql
import RDXROBOT.modules.sql.locks_sql as locksql
import RDXROBOT.modules.sql.notes_sql as sql

# from RDXROBOT.modules.rules import get_rules
import RDXROBOT.modules.sql.rules_sql as rulessql
from RDXROBOT import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from RDXROBOT.__main__ import DATA_IMPORT
from RDXROBOT.modules.connection import connected
from RDXROBOT.modules.helper_funcs.alternate import typing_action
from RDXROBOT.modules.helper_funcs.chat_status import user_admin
from RDXROBOT.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("RDXROBOT{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("RDXROBOT{}.backup".format(chat_id), "rb"),
        caption="❍ *sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ*\n❍ ᴄʜᴀᴛ ➛ `{}`\n❍ ᴄʜᴀᴛ ɪᴅ ➛ `{}`\n❍ ᴏɴ ➛ `{}`\n\n❍ ɴᴏᴛᴇ ➛ ᴛʜɪs `RDXROBOT-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("RDXROBOT{}.backup".format(chat_id))  # Cleaning file
